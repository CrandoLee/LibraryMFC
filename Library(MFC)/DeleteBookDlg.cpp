...
void DeleteBookDlg::OnQuery()
{
	...
	if (book.GetBookID() != -1)
	{
		...
		m_cListCtrl.InsertColumn(0, _T("1"));
...
BOOL DeleteBookDlg::OnInitDialog()
{
	...
	// TODO:  在此添加额外的初始化
	m_cListCtrl.InsertColumn(0, _T("ID"), LVCFMT_LEFT, 80);
...
// 	m_cListCtrl.InsertColumn(5, _T("入库日期"), LVCFMT_LEFT, 78);
// 	m_cListCtrl.InsertColumn(6, _T("库存"), LVCFMT_LEFT, 79);
